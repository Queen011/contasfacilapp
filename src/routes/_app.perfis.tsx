...
<body>
  ...
  <script>
    ...
    document.addEventListener('pointerup',(event)=>focusNative(event.target),true);document.addEventListener('touchend',(event)=>focusNative(event.target),true);$('nome').addEventListener('input',render);$('form').addEventListener('submit',(event)=>{event.preventDefault();if(busy)return;post({type:'submit',payload:{nome:$('nome').value,emoji,cor}})});window.addEventListener('message',(event)=>{const data=event.data||{};if(data.source!=='contasfacil-perfil-parent')return;if(data.type==='busy'){busy=Boolean(data.busy);$('saveBtn').disabled=busy;$('saveBtn').textContent=busy?'Salvando...':'+ Adicionar perfil';if(!busy)setTimeout(()=>$('nome').focus({preventScroll:false}),0)}if(data.type==='reset')reset()});render();
  </script>
</body>
</html>`;
}
